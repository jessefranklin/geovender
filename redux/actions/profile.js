      ...
      [
        {
          ...
          onPress: () => {
            ...
            f.database()
              .ref("profiles/" + f.auth().currentUser.uid + "/images")
              .set(array);
          }
        },
...
export function updateAbout(value) {
  return function(dispatch) {
    ...
    setTimeout(function() {
      f.database()
        .ref("profiles/" + f.auth().currentUser.uid)
      ...
      if (result) {
        Location.getCurrentPositionAsync({}).then(function(location) {
          ...
          Location.reverseGeocodeAsync(locObj).then(address => {
            f.database()
              .ref("profiles/" + f.auth().currentUser.uid)
              .update({ geocode: geocode, city: address[0].city });
            dispatch({ type: "GEOCODE", geocode: geocode });
          });
        });
      }
    });
